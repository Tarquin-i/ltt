...
const HTML_PATH = path.join(CLIENT_DIR, 'The Project Gutenberg eBook of The Art of Perfumery, by G.W. Septimus Piesse..html')
const IMAGES_SRC = path.join(CLIENT_DIR, 'The Project Gutenberg eBook of The Art of Perfumery, by G.W. Septimus Piesse._files')
const IMAGES_DEST = path.join(CLIENT_DIR, 'assets', 'images', 'book')
...
function writeChapterFiles(chapterHtmls) {
  ...
  CHAPTER_DEFS.forEach(def => {
    ...
    const filePath = path.join(CHAPTERS_DIR, `${def.id}.ts`)
